...
{
    __block BOOL res = NO;
    ZhDbHelper *zhDB = [ZhDbHelper shareInstance];
    [zhDB.dbQueue inDatabase:^(FMDatabase *db) {
...
+ (BOOL)createTable
{
    ...
    ZhDbHelper *zhDB = [ZhDbHelper shareInstance];
    [zhDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        NSString *tableName = NSStringFromClass(self.class);
...
//+ (BOOL)createTable
//{
//    FMDatabase *db = [FMDatabase databaseWithPath:[ZhDbHelper dbPath]];
//    if (![db open]) {
//        NSLog(@"数据库打开失败!");
//        return NO;
//    }
...
{
    //判断是否是JKBaseModel的子类
    for (ZhDBModel *model in array) {
        if (![model isKindOfClass:[ZhDBModel class]]) {
            return NO;
        }
    }
...
+ (NSArray *)findAll
{
    ...
    [zhDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            ZhDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
